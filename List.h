...
public:
    ...
    // @note Use the global `matchOption` and `match` instead of this member functions
    //
    // @todo the member functions do not type traits support, however the global ones
    // require `enable_if` to pass compilation, this is really wierd
    //
    // mimic pattern match
